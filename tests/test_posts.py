...
def test_get_one_post_not_exit(authorized_client, test_posts):
    res = authorized_client.get(f"/posts/8848")
...
def test_get_one_post(authorized_client, test_posts):
    ...
    post.Post.content == test_posts[0].content
    post.Post.title == test_posts[0].title

@pytest.mark.parametrize("title, content, published", [
    ("awesome new title", "awesome new contnent", True),
    ("favorite pizza", "honululu hawaii", False),
    ("tallest tree", "mantree", True)
    ])
...
def test_create_post_default_published_true(authorized_client, test_posts, test_user):
    res = authorized_client.post("/posts/", json={"title": "arbitrary title", "content": "adfad"})

    created_post = schema.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "arbitrary title"
    assert created_post.content == "adfad"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_posts(client, test_posts, test_user):
    res = client.post("/posts/", json={"title": "arbitrary title", "content": "adfad"})
    assert res.status_code == 401
...
def test_delete_post_non_exist(authorized_client, test_posts, test_user):
    res = authorized_client.delete(f"/posts/9843297485372")
    assert res.status_code == 404
...
def test_update_post_non_exist(authorized_client, test_posts, test_user):
    ...
    res = authorized_client.put(f"/posts/9843297485372", json=data)
    assert res.status_code == 404
